...
pltline.Source.Point1 = [0.0,   0.005, 0.0]
pltline.Source.Point2 = [0.076, 0.005, 0.0]


pv.UpdatePipeline(time=t[-1], proxy=pltline)
idat    = dsa.WrapDataObject(pv.servermanager.Fetch(pltline))
outarr=np.array([idat.Points[:,0],idat.PointData['C']])
np.savetxt("conc.dat", np.transpose(outarr), delimiter=" ")
