...
pltline.Source.Point1 = [0.0, 0.05, 0.0005]
pltline.Source.Point2 = [2.0, 0.05, 0.0005]


pv.UpdatePipeline(time=t[-1], proxy=pltline)
idat    = dsa.WrapDataObject(pv.servermanager.Fetch(pltline))
outarr=np.array([idat.Points[:,0],idat.PointData['p_rgh']])
np.savetxt("pres.dat", np.transpose(outarr), delimiter=" ")
